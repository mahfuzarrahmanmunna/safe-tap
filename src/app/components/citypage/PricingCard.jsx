...
function PricingCard({ activeProduct = "copper" }) {
  ...
  const currentProductData =
    allPricingData[activeProduct] || allPricingData.copper;
  ...
  useEffect(() => {
    const activeUsage = pricingData[usage]
      ? usage
      : Object.keys(pricingData)[0];
    const price = pricingData[activeUsage][tenure];

    setCurrentPrice(price);
    setCurrentSavings(
      pricingData[activeUsage].savings + (tenure === "360 days" ? 100 : 0),
    );
  ...
  return (
    <div className="hidden lg:block sticky top-24 right-0 w-[360px] h-fit z-40">
      ...
      >
        <div
          className={`p-5 bg-gradient-to-br ${isDark ? "from-cyan-950/20 to-transparent" : "from-cyan-50/50 to-transparent"}`}
        >
          <h2 className="text-xl font-black text-cyan-600 mb-0.5 tracking-tight">
            {currentProductData.name}
          </h2>
          <p
            className={`text-xs font-bold leading-tight mb-3 ${isDark ? "text-slate-300" : "text-slate-600"}`}
          >
          ...
          <div className="flex items-center gap-2 mb-2">
            <div className="flex -space-x-1.5">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="w-5 h-5 rounded-full border border-white bg-slate-300 overflow-hidden shadow-sm"
                >
                  <Image
                    width={20}
                    height={20}
        ...
        <div className="p-4">
          <div className="flex justify-between items-center mb-3">
            <h3
              className={`text-[11px] font-black uppercase tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}
            >
              1. Usage
            </h3>
            <button
              onClick={() => setIsModalOpen(true)}
              className="flex items-center gap-1 text-[9px] font-bold text-cyan-600 bg-cyan-50 px-2 py-0.5 rounded-full hover:bg-cyan-100 transition-colors"
            >
                  ...
                  ${
                    usage === item
                      ...
                      : isDark
                        ...
                        : "border-slate-50 bg-slate-50 text-slate-600 hover:border-cyan-200"
                  }`}
              >
                {item}
              </button>
        ...
        <div className="px-4 pb-4">
          <h3
            className={`text-[11px] font-black uppercase tracking-wider mb-3 ${isDark ? "text-slate-400" : "text-slate-500"}`}
          >
            2. Tenure
          </h3>
                ...
                className={`flex-1 py-2 rounded-lg text-[10px] font-bold border transition-all
                  ${
                    tenure === item
                      ...
                      : isDark
                        ? "border-slate-800 bg-slate-700/30 text-slate-400"
        ...
        <div className="px-4 pb-4 flex justify-between items-center">
          <div>
            <p className="text-[9px] font-black text-emerald-500 uppercase tracking-tighter mb-1">
              7-days FREE TRIAL
            </p>
        ...
        <button
          ...
          className="w-full py-4 bg-cyan-600 hover:bg-cyan-700 text-white font-black text-lg transition-all active:scale-95 shadow-inner"
        >
          Try It Free
        </button>
      </div>
