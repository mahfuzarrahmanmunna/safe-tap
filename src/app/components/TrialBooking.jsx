...
function TrialBooking() {
  ...
  const handleRegistrationSuccess = (data) => {
    console.log(
      "Registration successful, showing phone verification modal:",
      data,
    );
  ...
  return (
    <section
      className={`py-24 relative overflow-hidden transition-all duration-700 ${isDark ? "bg-[#020617]" : "bg-slate-50"}`}
    >
      {/* Background Elements */}
      ...
      <div className="max-w-5xl mx-auto px-6 text-center">
        <div className="space-y-6 mb-12">
          ...
          >
            Start Your <span className="text-cyan-500">7-Day Risk-Free</span>{" "}
            Trial
          </h2>
        </div>
            ...
            <div className="w-full md:w-1/2 p-8 md:p-12 text-left flex flex-col justify-center">
              <div className="space-y-6">
                <div className="relative">
                  <label
                    className={`block text-xs font-bold uppercase tracking-widest mb-3 ml-2 ${isDark ? "text-slate-500" : "text-slate-400"}`}
                  >
                    Choose Your City
                  </label>
                  <div className="relative">
      ...
      <button
        ...
        className="fixed bottom-4 right-4 px-4 py-2 bg-cyan-600 text-white rounded-lg z-50"
      >
        Test Phone Modal
      </button>
    </section>
